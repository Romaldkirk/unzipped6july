...
def fetch_studies(sponsor: str, max_pages: int = 3) -> List[dict]:
    ...
    row = c.fetchone()
    if False:  # 🔧 Cache disabled for test run
...
def is_sponsor_match(trial: dict, company: str) -> bool:
    try:
        ...
        if not sponsor:
            return False  # Skip blank sponsors safely
...
def extract_trial_info(studies: List[dict], company: str) -> Tuple[int, str, str, str]:
    for s in studies:
        sponsor = s.get("protocolSection", {}).get("identificationModule", {}).get("organization", {}).get("orgName", "")
        print(f"    [LOG] Sponsor field: '{sponsor}'")
